...
"""
This is a modification of the "https://hypnettorch.readthedocs.io/en/latest/_modules/hypnettorch/data/special/gaussian_mixture_data.html#GaussianData"
hypenettorch file to include validation data.
"""
import numpy as np
...
def get_gmm_tasks(means=DEFAULT_MEANS, covs=DEFAULT_VARIANCES, num_train=100,
                 num_test=100, num_val=100, map_functions=None, rseed=None):
    ...
    -----------
        ...
        num_test: int
            Number of test samples per task.
        map_functions: Optional)
            A list of "map_functions", one for each task.
        rseed: int
            See argument ``rseed`` of class :class:`GaussianData`. The
    ...
    -------
    List
        A list of objects of class :class:`GaussianData`.
    """
    ...
    for i in range(len(means)):
        ret.append(GaussianData(mean=means[i], cov=covs[i], num_train=num_train,
            num_test=num_test, num_val=num_val, map_function=map_functions[i],
...
class GaussianData(Dataset):
    ...
    def __init__(self, mean=np.array([0, 0]), cov=0.05**2 * np.eye(2),
                 num_train=100, num_test=100, num_val=100, map_function=None, rseed=None):
        ...
        -----------
            ...
            map_function: Optional
                A function handle that receives input samples and maps them to output samples. 
                If not specified, the density function will be used as map function.
            rseed: int
                If ``None``, the current random state of numpy is used
        ...
        val_x   = rand.multivariate_normal(mean, cov, size=num_val)
        test_x  = rand.multivariate_normal(mean, cov, size=num_test)

        # Clip samples to ensure non-negativity
        train_x = np.clip(train_x, 0, None)
        val_x   = np.clip(val_x, 0, None)
        test_x  = np.clip(test_x, 0, None)
        ...
        self._data['in_data'] = np.vstack([train_x, test_x, val_x])
        self._data['in_shape'] = [n_x]
        self._data['out_data'] = np.vstack([train_y, test_y, val_y])
        ...
        self._data['val_inds'] = np.arange(num_train + num_test, num_train + num_test + num_val)
    ...
    def plot_samples(self, title, inputs, outputs=None, predictions=None,
        ...
        ------------
            title: str
                The title of the whole figure.
            inputs: np.ndarray
                A 2D numpy array, where each row is an input sample.
            outputs: np.ndarray, Optional
                 A 2D numpy array of actual dataset targets.
            predictions: np.ndarray, Optional
                A 2D numpy array of predicted output
                samples (i.e., output predicted by a neural network).
            num_samples_per_row: int
                Maximum number of samples plotted per row in the generated figure.
            show: bool
                Whether the plot should be shown.
            filename: str, Optional
                If provided, the figure will be stored under this filename.
            interactive: bool
                Turn on interactive mode. We mainly
                ...
                effect.
            figsize: Tuple
                A tuple, determining the size of the figure in inches.
