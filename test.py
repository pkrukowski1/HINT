    ...
    def test_apply_mask_to_weights_of_network(self):
        ...
        test_weights_1 = nn.ParameterList([
            torch.nn.Parameter(torch.Tensor(
                [[1.2, -1, 5.], [1.8, -2.1, 3.]]).to(
                    device=self.device)),
            torch.nn.Parameter(torch.Tensor(
                [[1.35, 1.9, 2.3, 2.4]]).to(device=self.device)),
            torch.nn.Parameter(torch.Tensor(
                [[-0.6, 0.3, 0.4, -0.85],
                 [1.2, 1.8, 1.9, 1.5],
                 [-1., 2., -0.1, 0.8]]).to(device=self.device))
        ])
        test_1_BN = None
        ...
        test_mask_1 = [
            torch.Tensor(
                [[1, 0, 1], [0, 1, 1]]).to(device=self.device),
            torch.Tensor(
                [[1, 0, 0, 1]]).to(device=self.device),
            torch.Tensor(
                [[1, 0, 0, 0],
                 [0, 0, 0, 0],
        ...
        gt_mask_1 = [
            torch.Tensor(
                [[1.2, 0., 5.], [0., -2.1, 3.]]).to(device=self.device),
            torch.Tensor(
                [[1.35, 0., 0., 2.4]]).to(device=self.device),
        ...
        test_weights_2 = nn.ParameterList([
            torch.nn.Parameter(torch.Tensor(
                [[0.75, 2.2, -1.85, 2.37]]).to(device=self.device)),
            torch.nn.Parameter(torch.Tensor(
                [[0., 1.1, 0.2, 0.7]]).to(device=self.device)),
            torch.nn.Parameter(torch.Tensor(
                [[1.2, -1, 5.], [1.8, -2.1, 3.]]).to(
                    device=self.device)),
            torch.nn.Parameter(torch.Tensor(
        ...
        test_2_BN = nn.ParameterList([
            torch.nn.Parameter(torch.Tensor(
                [[0.1, 0.2, 0.3, 0.4]]).to(device=self.device))
        ])
        gt_mask_2 = [
            torch.Tensor(
                [[0.75, 2.2, -1.85, 2.37]]).to(device=self.device),
