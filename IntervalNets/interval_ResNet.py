...
# Is is interval version of ResNet-18 based on https://github.com/gmum/HyperMask/blob/main/ResNetF.py
...
287f934ea5fa00d4345c2cccecf3552e2b1c33e3/model/resnet.py#L66>`__, i.e., it uses
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._context_mod_post_activation = kwargs["context_mod_post_activation"]
        ...
        if len(blocks_per_group) != 4:
            raise ValueError(
                'Option "blocks_per_group" must be a list of 4 ' + "integers."
            )
        self._num_blocks = blocks_per_group
        if len(num_feature_maps) != 5:
            raise ValueError(
                'Option "num_feature_maps" must be a list of 5 ' + "integers."
            )
        ...
        self._num_main_conv_layers = 1 + int(
            np.sum(
                [
                    self._num_blocks[i] * (3 if self._bottleneck_blocks else 2)
                    for i in range(4)
                ]
            )
        )
        ...
        self._internal_params = (
            None if no_weights and self._context_mod_no_weights else nn.ParameterList()
        )
        ...
        #################################
        self._context_mod_layers = nn.ModuleList() if self._use_context_mod else None
            ...
            for i in range(4):  # For each transition between conv groups.
                ...
                if not no_weights:
                    self._internal_params.append(
                        nn.Parameter(torch.Tensor(*skip_1x1_shape), requires_grad=True)
                    )
        ...
        if verbose:
            if self._use_context_mod:
                ...
                cm_num_params = MainNetInterface.shapes_to_num_weights(cm_param_shapes)

            print(
                'Creating a "%s" with %d weights' % (str(self), self.num_params)
                + (
